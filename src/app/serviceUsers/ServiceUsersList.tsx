...
const ServiceUsersList: React.FC = () => {
  const [statusFilter, setStatusFilter] = useState<'admitted' | 'discharged' | 'all'>('all');
  const [sortBy, setSortBy] = useState<string>('name');
  const [order, setOrder] = useState<'asc' | 'desc'>('desc');
  const [groupByWard, setGroupByWard] = useState<boolean>(false);

  const { data, fetchNextPage, hasNextPage, isFetchingNextPage } = useServiceUsers({
    statusFilter,
    sortBy,
    order,
    groupByWard,
    ...
    <div>
      ...
      <div className="overflow-x-auto">
        {!groupByWard ? (
          ...
          <table className="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
            <thead className="bg-gray-800 text-white">
              <tr>
                ...
                <th className="py-3 px-4 uppercase font-semibold text-sm">Ward</th>
          ...
          data?.pages.map((page, pageIndex) => (
            <div key={pageIndex}>
              {Object.entries(page.serviceUsers).map(([wardName, users]: [string, ServiceUser[]]) => (
                <div key={wardName}>
                  <h2 className="text-xl font-semibold mt-4 mb-2">{wardName}</h2>
                  <table className="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
                    <thead className="bg-gray-800 text-white">
                      <tr>
                        <th className="py-3 px-4 uppercase font-semibold text-sm">Name</th>
